...
// the the logger will write to a `upgrade-tool.log` file inside the `upgrade-tool` subdirectory of
// the user cache directory. For example, in a Linux system the default file will be
// `~/.cache/upgrade-tool/upgrade-tool.log`
...
// SetFile sets the file that the logger will write to. This is optional, and if not specified the
// the logger will write to a `upgrade-tool.log` file inside the `upgrade-tool` subdirectory of the
// user cache directory. For example, in a Linux system the default file will be
// `~/.cache/upgrade-tool/upgrade-tool.log`
...
func (b *LoggerBuilder) openDefaultFile() (result io.Writer, err error) {
	...
	dir = filepath.Join(dir, "upgrade-tool")
	_, err = os.Stat(dir)
	...
	file := filepath.Join(dir, "upgrade-tool.log")
